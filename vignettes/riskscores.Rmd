...
The `breastcancer` dataset is already ready to go. However, we'll still need to split out our data into a matrix with all covariates (`X`) and a vector with the outcome data (`y`). In this case, the first column in our dataset contains the outcome variable (`benign`). We also add to `X` a column representing the intercept (i.e. a column containing all 1s).
