...
#'  \item{results}{Dataframe containing a summary of deviance, accuracy, and auc for each
...
#' @export
cv_annealscore <- function(X, y, weights = NULL, beta = NULL, a = -10, b = 10,
                        max_iters = 1000, tol= 1e-5, nlambda = 25,
  ...
  fold_fcn <- function(l0, foldid){
    ...
    mod <- annealscore(X_train, y_train, gamma = NULL, beta = NULL,
                    weights = weight_train, lambda0 = l0, a = a, b = b,
                    max_iters = max_iters, tol = tol)
    res <- get_metrics_internal_annealscore(mod, X_train, y_train)
    non_zeros <- sum(mod$beta != 0) - 1
  ...
  if (parallel) {
    ...
    outlist = foreach::foreach(i = 1:nrow(res_df), .export="get_metrics_internal_annealscore") %dopar%
      {
  ...
  full_fcn <- function(l0) {
    mod <- annealscore(X, y,  gamma = NULL, beta = NULL,
  ...
  # Find lambda_min and lambda1_se using auc
  ...
  cv_obj <- list(results = res_df_summary, res_df = res_df, lambda_min = lambda_min,
